...
//       if(to_be_approved_leaves.length === 0) { to_be_approved_leaves = undefined}
//         res.render('users',{
...
router.get('/', function(req, res) {
    ...
    .then(function(company, to_be_approved_leaves){
        res.render('users', {
...
function get_and_validate_user_parameters(args) {
    ...
    if(req.param('admin') && req.param('approver')){
      req.session.flash_error(
        'User can only be approver or admin, not both.'
      );
    }
