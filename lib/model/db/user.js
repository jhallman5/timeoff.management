...
module.exports = function(sequelize, DataTypes) {
  ...
  var User = sequelize.define("User", {
      ...
      approver : {
          ...
          allowNull    : false,
          defaultValue : false,
          comment      : 'Indicate if account can approve company leaves',
...
function get_instance_methods(sequelize) {
  ...
  return {
    ...
    promise_users_I_can_manage : function(){
        ...
        // Check if current user is admin, then fetch all users from company
        if ( this_user.is_admin() || this_user.is_approver() ) {
...
function withAssociations() {
  ...
  this.associate = function(models){
    ...
    // models.User.belongsToMany(models.Leave, {
    //   as : 'user',
    //   foreignKey : 'userId',
    //   through: 'UserIds_LeaveRequests'
    // });
